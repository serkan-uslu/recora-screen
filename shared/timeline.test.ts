...
test("source mapping preserves all synchronized tracks across cuts and boundaries", () => {
  ...
  assert.deepEqual(sourceRanges(segments, 500, 2000), [
    { startMs: 500, endMs: 1000 },
    { startMs: 3000, endMs: 4000 },
  ]);
  assert.deepEqual(subtractRanges(segments, sourceRanges(segments, 500, 2000)), [
    { startMs: 0, endMs: 500 },
...
test("mixed playback speeds map both directions and retain speeds through range edits", () => {
  const segments = [
    ...
    { startMs: 3000, endMs: 7000, speed: 2 },
  ];
  assert.equal(duration(segments), 4000);
  ...
  assert.equal(timelineTime(segments, 2000), null);
  assert.deepEqual(sourceRanges(segments, 1000, 3000), [
    { startMs: 500, endMs: 1000 },
    { startMs: 3000, endMs: 5000 },
  ]);
  assert.deepEqual(sliceSegments(segments, 1000, 3000), [
    { startMs: 500, endMs: 1000, speed: 0.5 },
    { startMs: 3000, endMs: 5000, speed: 2 },
  ]);
  assert.deepEqual(subtractRanges(segments, [{ startMs: 250, endMs: 750 }]), [
    { startMs: 0, endMs: 250, speed: 0.5 },
    { startMs: 750, endMs: 1000, speed: 0.5 },
    segments[1],
  ]);
  assert.deepEqual(outputRanges(segments, { startMs: 500, endMs: 5000 }), [
    { startMs: 1000, endMs: 2000 },
    { startMs: 2000, endMs: 3000 },
  ]);
  const fractional = Array.from({ length: 100 }, (_, i) => ({
    startMs: i * 1000,
    endMs: (i + 1) * 1000,
    speed: 0.25 + i * 0.073,
  }));
