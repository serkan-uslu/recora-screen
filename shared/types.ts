...
export type CanvasSettings = {
  ...
  color: string; gradientTo: string; gradientAngle: number;
  wallpaper: 'aurora' | 'sunset' | 'ocean' | 'dusk'; assetId?: string;
  blur: number; padding: number; radius: number; shadow: number;
  frame: 'none' | 'minimal' | 'browser'; title: string;
};
...
export type EditState = {
  segments: TimelineSegment[]; camera: CameraSettings; zooms: Zoom[]; overlays: Overlay[];
  canvas?: CanvasSettings; autoZoom?: AutoZoomSettings;
...
export function defaultCanvas(): CanvasSettings {
  return { aspectRatio: '16:9', background: 'gradient', color: '#111827', gradientTo: '#115e59', gradientAngle: 135,
    wallpaper: 'aurora', blur: 0, padding: 0.06, radius: 0.025, shadow: 0.35, frame: 'none', title: '' };
}
export function defaultAutoZoom(): AutoZoomSettings {
  return { enabled: true, scale: 1.7, leadMs: 400, holdMs: 1600, gapMs: 600, motion: 'gentle', followCursor: true };
}
