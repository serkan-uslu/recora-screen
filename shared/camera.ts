import type { CameraLayoutSettings, Project, Range } from './types.js';

export type CameraRun = CameraLayoutSettings & Range;
export const cameraLayoutSettings = ({ shape, x, y, size, shadow }: CameraLayoutSettings): CameraLayoutSettings => ({ shape, x, y, size, shadow });
...
export function cameraOutputLayouts(project: Pick<Project, 'edits'>): CameraRun[] {
  ...
  for (const segment of segments) {
    ...
    while (source < segment.endMs) {
      ...
      const end = Math.min(segment.endMs, active ? layout.endMs : layout?.startMs ?? segment.endMs);
...
export function cameraAt(project: Pick<Project, 'edits'>, timeMs: number): CameraLayoutSettings {
  ...
  for (let index = 1; index < runs.length; index++) {
    const before = runs[index - 1]!, after = runs[index]!;
  ...
  return cameraLayoutSettings(runs.find((run) => timeMs < run.endMs) ?? runs.at(-1) ?? project.edits.camera);
}
