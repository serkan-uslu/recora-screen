...
const systemPath = value => { const normalized = path.normalize(value); return normalized.startsWith('/System/Library/') || normalized.startsWith('/usr/lib/'); };
const inside = (root, file) => file === root || file.startsWith(root + path.sep);
const dependencies = output => output.split('\n').flatMap(line => {
  const match = line.match(/^\s+(.+?)\s+\(compatibility version/);
  return match ? [match[1]] : [];
});
const runpaths = output => [...output.matchAll(/cmd LC_RPATH\s+cmdsize \d+\s+path (.+?) \(offset \d+\)/g)].map(match => match[1]);
const portableReference = value => systemPath(value) || /^@(rpath|loader_path|executable_path)(\/|$)/.test(value);

function run(executable, args, options = {}) {
  const result = spawnSync(executable, args, { encoding: 'utf8', timeout: 30_000, maxBuffer: 8_000_000, ...options });
  if (result.error || result.status !== 0) {
    throw new Error(`${path.basename(executable)} ${args.join(' ')} failed: ${result.error?.message || result.stderr?.trim() || result.stdout?.trim() || `exit ${result.status}`}`);
  }
  ...
  async function walk(directory) {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) {
        assert(inside(root, await realpath(file)), `Bundle symlink points outside the app: ${file}`);
        continue; // A framework's versioned target is inspected through its real directory.
      }
      if (entry.isDirectory()) await walk(file);
      else if (entry.isFile()) {
        const handle = await open(file, 'r'), header = Buffer.alloc(4);
        try { await handle.read(header, 0, 4, 0); } finally { await handle.close(); }
        if (['feedface', 'cefaedfe', 'feedfacf', 'cffaedfe', 'cafebabe', 'bebafeca', 'cafebabf', 'bfbafeca'].includes(header.toString('hex'))) files.push(file);
      }
    }
  }
  await walk(root);
...
async function verify(input) {
  ...
  assert(typeof info.CFBundleExecutable === 'string' && path.basename(info.CFBundleExecutable) === info.CFBundleExecutable, 'Invalid CFBundleExecutable.');
  assert(info.CFBundleIdentifier === 'com.screenrecorder.desktop', 'Unexpected bundle identifier.');
  assert(Number.parseFloat(info.LSMinimumSystemVersion) >= 15, 'The app must declare macOS 15 or later.');
  const main = path.join(contents, 'MacOS', info.CFBundleExecutable);
  const resources = path.join(contents, 'Resources');
  const node = path.join(resources, 'bin/node'), whisper = path.join(resources, 'bin/whisper-cli');
  const native = path.join(contents, 'Frameworks/libscreenrec.dylib');
  for (const relative of ['server.mjs', 'mcp.mjs', 'THIRD_PARTY_NOTICES.txt', 'bin/NODE-LICENSE', 'bin/WHISPER-LICENSE', 'bin/node-version', 'bin/whisper-version']) {
    const file = path.join(resources, relative);
    assert((await stat(file)).isFile() && (await stat(file)).size > 0, `Missing or empty bundled resource: ${relative}`);
  }
  ...
  for (const required of [main, node, whisper, native]) assert(binaries.includes(required), `Expected a Mach-O binary: ${required}`);
  const metadata = new Map(binaries.map(file => [file, {
  ...
  const issues = [];
  const expand = (reference, file) => reference
  ...
  for (const [file, data] of metadata) {
    ...
    for (const reference of [...data.dependencies, ...data.rpaths]) {
      if (!portableReference(reference)) issues.push(`${label} contains a nonportable load path: ${reference}`);
    }
    const rpaths = [...data.rpaths, ...metadata.get(main).rpaths];
    for (const dependency of data.dependencies) {
      if (portableReference(dependency) && !await resolves(dependency, file, rpaths)) issues.push(`${label} has an unresolved bundled dependency: ${dependency}`);
    }
    for (const architecture of metadata.get(main).architectures) {
      if (!data.architectures.includes(architecture)) issues.push(`${label} lacks the main executable's ${architecture} architecture.`);
    }
    try { run('/usr/bin/codesign', ['--verify', '--strict', file]); }
    catch (error) { issues.push(error.message); }
  }
  try { run('/usr/bin/codesign', ['--verify', '--deep', '--strict', '--verbose=2', app]); }
  catch (error) { issues.push(error.message); }
  assert.equal(issues.length, 0, `Bundle verification failed:\n${issues.map(issue => `- ${issue}`).join('\n')}`);
  ...
  for (const key of Object.keys(cleanEnv)) if (key.startsWith('DYLD_') || key.startsWith('NODE_')) delete cleanEnv[key];
  assert(/native bundle ready/.test(run(main, ['--bundle-check'], { env: cleanEnv })), 'The main app could not load its native library.');
  ...
  for (const script of ['server.mjs', 'mcp.mjs']) run(node, ['--check', path.join(resources, script)], { env: cleanEnv });
  const help = run(whisper, ['--help'], { env: cleanEnv });
  assert(/usage:|--model/i.test(help), 'The bundled Whisper executable did not return its help text.');
...
if (process.argv.includes('--self-test')) {
  assert.deepEqual(dependencies('app:\n\t@rpath/lib test.dylib (compatibility version 1.0.0, current version 1.0.0)\n'), ['@rpath/lib test.dylib']);
  assert.deepEqual(runpaths('Load command 4\n          cmd LC_RPATH\n      cmdsize 48\n         path @executable_path/../Frameworks (offset 12)\n'), ['@executable_path/../Frameworks']);
  for (const reference of ['/opt/homebrew/lib/lib.dylib', '/usr/local/lib/lib.dylib', '/Users/dev/.cache/lib.dylib', 'relative/lib.dylib', '/usr/lib/../../opt/homebrew/lib/lib.dylib']) assert(!portableReference(reference));
  for (const reference of ['/usr/lib/libSystem.B.dylib', '/System/Library/Frameworks/AppKit.framework/AppKit', '@rpath/libscreenrec.dylib', '@loader_path/lib.dylib']) assert(portableReference(reference));
