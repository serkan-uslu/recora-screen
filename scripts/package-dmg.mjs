...
assert(typeof version === 'string' && /^\d+\.\d+\.\d+(?:-[\w.-]+)?(?:\+[\w.-]+)?$/.test(version), 'package.json must contain a safe semantic version.');
...
execFileSync(process.execPath, [path.join(root, 'scripts/verify-bundle.mjs'), app], { stdio: 'inherit' });
...
try {
  execFileSync('/usr/bin/ditto', [app, path.join(staging, 'Screen Recorder.app')], { stdio: 'inherit' });
  await symlink('/Applications', path.join(staging, 'Applications'));
  execFileSync('/usr/bin/hdiutil', ['create', '-volname', 'Screen Recorder', '-srcfolder', staging, '-format', 'UDZO', pending], { stdio: 'inherit' });
