...
execFileSync("/usr/bin/say", [
  "-o",
  original,
  "Welcome to Screen Recorder. Create a project, record your screen, and edit your video. Your recordings stay on your computer.",
]);
execFileSync("/usr/bin/afconvert", ["-f", "WAVE", "-d", "LEI16@16000", "-c", "1", original, audio]);
const segments = await ai.transcribe(
  "base",
  audio,
  path.join(directory, "transcript"),
  "en",
  signal,
  () => {},
);
...
assert.ok(
  segments.every((s) => s.endMs > s.startMs && s.startMs >= 0),
  "Timestamps should be ordered positive intervals.",
);
