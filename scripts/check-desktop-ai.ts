...
assert(
  process.env.SCREENREC_DATA_DIR && dataDir.startsWith(path.resolve(".cache") + path.sep),
  "Run against an isolated SCREENREC_DATA_DIR inside this repository’s .cache directory.",
);
...
assert.equal(
  await hashFile(cachedModel),
  models.find((m) => m.id === "base")!.sha256,
  "Prepare a verified Base model with npm run test:ai first.",
);
...
const probe = JSON.parse(
  execFileSync(
    "ffprobe",
    ["-v", "error", "-show_entries", "format=duration", "-of", "json", speech],
    { encoding: "utf8" },
  ),
);
...
execFileSync(
  ...
  [
    ...
    speech,
    "-filter_complex",
    "[0:v]drawbox=x=0:y=ih*0.75:w=iw:h=ih*0.25:color=0x18201a:t=fill[v];[1:a]adelay=2000:all=1[a]",
    "-map",
    "[v]",
    "-map",
    "[a]",
...
function subtitleRanges(text: string) {
  const time = (hours: string, minutes: string, seconds: string, milliseconds: string) =>
    Number(hours) * 3_600_000 +
    Number(minutes) * 60_000 +
    Number(seconds) * 1000 +
    Number(milliseconds);
  return [
    ...text.matchAll(/(\d{2}):(\d{2}):(\d{2})[,.](\d{3}) --> (\d{2}):(\d{2}):(\d{2})[,.](\d{3})/g),
  ].map((m) => ({ startMs: time(m[1], m[2], m[3], m[4]), endMs: time(m[5], m[6], m[7], m[8]) }));
}

try {
  assert((await call<{ nativeAvailable: boolean }>("app.capabilities")).nativeAvailable);
  let project = await call<Project>("project.import", {
    path: fixture,
    name: "Local AI acceptance fixture",
  });
  assert(
    project.source?.microphone,
    "Import must expose the audio track for native speech extraction.",
  );
  await finish(
    await call<Job>("ai.transcribe", { projectId: project.id, model: "base", language: "en" }),
  );
    ...
    original.length > 0 &&
      original.every(
        (segment) =>
          ...
          segment.endMs <= project.source!.durationMs,
      ),
  );
  ...
  const initialDuration = duration(project.edits.segments),
    cutMs = 1000;
  project = await call<Project>("timeline.apply", {
    projectId: project.id,
    expectedRevision: project.revision,
    operations: [{ type: "cut", startMs: 0, endMs: cutMs }],
  });
  assert.equal(duration(project.edits.segments), initialDuration - cutMs);
  assert.deepEqual(
    project.transcript,
    original,
    "Cuts must preserve source-time transcript segments.",
  );
  const expected = original
    .filter((segment) => segment.endMs > cutMs)
    .map((segment) => ({
      startMs: Math.round(Math.max(0, segment.startMs - cutMs)),
      endMs: Math.round(segment.endMs - cutMs),
    }));
  for (const format of ["srt", "vtt"]) {
    const output = path.join(artifacts, `shifted-captions.${format}`);
    const result = await call<{ text: string; path: string }>("transcript.export", {
      projectId: project.id,
      format,
      path: output,
    });
    assert.deepEqual(
      subtitleRanges(result.text),
      expected,
      `${format} timestamps must shift after removing the first second.`,
    );
  ...
  const frame = await client.callTool({
    ...
    arguments: {
      projectId: project.id,
      timeMs: Math.min(duration(project.edits.segments) - 1, expected[0].startMs + 800),
    },
  });
  const content = frame.content;
  assert(
    !frame.isError && Array.isArray(content),
    "Native caption composition should return a preview image through MCP.",
  );
  ...
  const pixels = execFileSync(
    ...
    [
      ...
      preview,
      "-vf",
      "crop=iw*0.8:ih*0.2:iw*0.1:ih*0.75,format=rgb24",
      "-f",
      "rawvideo",
      "-",
    ],
  ...
  for (let i = 0; i + 2 < pixels.length; i += 3)
    if (pixels[i] > 215 && pixels[i + 1] > 215 && pixels[i + 2] > 215) lightPixels++;
  assert(
    lightPixels > 20,
    `Caption preview has no visible white glyphs (${lightPixels} light pixels). Inspect ${preview}.`,
  );
  await fs.writeFile(
  ...
  console.log(
    `Desktop local AI passed: native audio preparation → bundled Whisper Base → ${original.length} timed segments; source transcript preserved after cut; shifted SRT/VTT; native caption preview image.\nTranscript: ${text}\nProject kept: ${project.name}\nArtifacts: ${artifacts}`,
  );
} finally {
