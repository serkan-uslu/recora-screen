...
if (process.platform !== 'darwin' || !['arm64', 'x64'].includes(arch)) throw new Error('Desktop bundles currently require macOS arm64 or x64.');
...
if (await readFile(marker, 'utf8').catch(() => '') === `${version}-${arch}` && await access(path.join(root, 'node'), constants.X_OK).then(() => true, () => false)) process.exit(0);
const base = `https://nodejs.org/dist/v${version}`;
const checksums = await fetch(`${base}/SHASUMS256.txt`).then(r => { if (!r.ok) throw new Error(`Node checksums: ${r.status}`); return r.text(); });
