...
const licenseName =
  /^(licen[cs]e|copying|copyright|notice|unlicense|third[_-]?party[_-]?notices?)([._-].*)?$/i;
...
async function licenseFiles(directory, declaredFile) {
  ...
  async function scan(dir, prefix = "") {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (
        ...
        !/\.(rs|js|ts|json)$/i.test(entry.name)
      ) {
        result.set(relative, await readFile(path.join(dir, entry.name), "utf8"));
      } else if (
        entry.isDirectory() &&
        /^(licenses?|licences?|legal|third[-_]?party[-_]?licenses)$/i.test(entry.name)
      ) {
  ...
  if (declaredFile)
    result.set(
      path.basename(declaredFile),
      await readFile(path.resolve(directory, declaredFile), "utf8"),
    );
  return result;
}
...
async function rustNotices() {
  ...
  try {
    await access(cargoHome, constants.W_OK);
  } catch {
    cargoHome = path.join(root, ".cache", "cargo");
  }
  ...
  const packages = metadata.packages
    .filter((pkg) => included.has(pkg.id) && pkg.id !== metadata.resolve.root)
  ...
  for (const pkg of packages) {
    ...
    if (pkg.source?.startsWith("registry+"))
      details.push(
        `Source code: https://crates.io/api/v1/crates/${pkg.name}/${pkg.version}/download`,
      );
    ...
    let provenance;
    try {
      provenance = await json(path.join(overrideDir, "SOURCE.json"));
    } catch (error) {
    ...
    if (provenance) {
      ...
      if (
        provenance.package !== pkg.name ||
        provenance.version !== pkg.version ||
        provenance.revision !== vcs.git.sha1
      ) {
      ...
      for (const source of provenance.files) {
        ...
        if (sha256(text) !== source.sha256)
          throw new Error(
            `Notice checksum mismatch for ${pkg.name}@${pkg.version}: ${source.file}`,
          );
...
async function main() {
  ...
  for (const [name, file, versionFile] of [
    ...
    section(
      name,
      [
        `Bundled version/revision: ${(await readFile(path.join(root, "resources", "bin", versionFile), "utf8")).trim()}`,
      ],
  ...
  console.log(
    `Bundled notices: ${npmCount} npm packages, ${rust.count} Rust dependencies; ${Buffer.byteLength(output)} bytes.`,
  );
}
