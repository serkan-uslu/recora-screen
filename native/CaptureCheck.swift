...
@main struct CaptureCheck {
    ...
    @MainActor static func main() async {
        ...
        assert(typingPosition(point: outside, lastInteraction: last, captureRect: rectangle, contentRect: content, capturedWindow: 12, focusedWindow: 12) == last)
        assert(typingPosition(point: outside, lastInteraction: nil, captureRect: rectangle, contentRect: content, capturedWindow: 12, focusedWindow: 12) == CGPoint(x: 0.5, y: 0.5))
        assert(typingPosition(point: outside, lastInteraction: last, captureRect: rectangle, contentRect: content, capturedWindow: 12, focusedWindow: 13) == nil)
        assert(typingPosition(point: outside, lastInteraction: nil, captureRect: rectangle, contentRect: content, capturedWindow: nil, focusedWindow: 12) == nil)
