...
Version 0.1.0 has been built as an Apple Silicon development app and DMG. The local package now uses an Apple Development certificate to keep its identity stable during development. Developer ID signing, notarization, and the remaining hardware acceptance checks are still required for public distribution.
