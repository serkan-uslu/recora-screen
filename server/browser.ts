...
const desktop = await client.connect(false).then(
  () => true,
  () => false,
);
const server = createServer((request, response) => {
  void (async (request, response) => {
    ...
    if (
      ...
      (origin &&
        !["http://127.0.0.1:1420", "http://localhost:1420", "http://127.0.0.1:4319"].includes(
          origin,
        ))
    ) {
    ...
    try {
      ...
      if (url.pathname === "/api/command" && request.method === "POST") {
        ...
        const result = desktop
          ? await client.call(input.method, input.params)
          : await service.command(input.method, input.params);
        response.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
        response.end(JSON.stringify({ id: input.id, result }));
        return;
      }
      ...
      response.end(JSON.stringify({ error: errorOf(error) }));
    }
  })(request, response);
});
server.listen(4319, "127.0.0.1", () =>
  console.error(
    `Browser development service on 127.0.0.1:4319 (${desktop ? "connected to desktop" : "isolated projects; native media unavailable"})`,
  ),
);
process.on("SIGINT", () => {
  client.close();
  server.close();
  void service.flush().then(() => process.exit(0));
});
