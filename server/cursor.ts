...
const eventSchema = z.object({ tMs: time, x: finite, y: finite, click: z.boolean().optional(), kind: z.enum(['click','drag','typing']).optional() }).strict();
...
export async function cursorClicks(file: string): Promise<CursorEvent[]> {
  ...
  let anchor: CursorEvent | undefined, lastTyping = -Infinity, spacingMs = 0;
  function retain(event: CursorEvent) {
    ...
    while (clicks.length > 10000) {
      ...
      clicks = clicks.filter(candidate => {
        ...
        last = candidate.tMs; return true;
      });
    }
  }
      ...
      if (quoted) {
        ...
        if (onScreen) {
          if (event.kind === 'typing') {
            if (event.tMs - lastTyping >= 600) { retain(event); lastTyping = event.tMs; }
          } else if (event.kind === 'drag') { retain(event); anchor = event; }
          else if (event.kind === 'click' && (event.click === undefined || !pressed) || event.click && !pressed) { retain(event); anchor = event; }
          else if (event.click && pressed && anchor && event.tMs - anchor.tMs >= 120 && Math.hypot(event.x - anchor.x, event.y - anchor.y) >= 0.025) {
            retain({ ...event, kind: 'drag' }); anchor = event;
          }
        }
