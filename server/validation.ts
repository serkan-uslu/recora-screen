...
export const segment = z.object({ ...rangeShape, speed: speed.optional() }).strict().refine(r => r.endMs > r.startMs, 'End must be after start');
...
export const zoom = z.object({ ...rangeShape, id, scale: finite.min(1).max(4), x: unit, y: unit, motion: motion.optional(), followCursor: z.boolean().optional() }).strict();
export const autoZoom = z.object({ enabled: z.boolean(), scale: finite.min(1).max(4), leadMs: time.max(10000), holdMs: time.min(200).max(30000), gapMs: time.max(30000), motion, followCursor: z.boolean() }).strict();
export const canvas = z.object({ aspectRatio: z.enum(['source','16:9','9:16','1:1','4:5']), background: z.enum(['hidden','color','gradient','wallpaper','image']), color, gradientTo: color, gradientAngle: finite.min(0).max(360), wallpaper: z.enum(['aurora','sunset','ocean','dusk']), assetId: id.optional(), blur: finite.min(0).max(60), padding: finite.min(0).max(0.2), radius: finite.min(0).max(0.1), shadow: unit, frame: z.enum(['none','minimal','browser']), title: z.string().max(1000) }).strict();
...
export const editState = z.object({ segments: z.array(segment).max(10000).refine(segments => segments.every((segment, index) => !index || segment.startMs >= segments[index - 1]!.endMs), 'Segments must be in source order without overlaps'), camera, zooms: z.array(zoom).max(10000), overlays: z.array(overlay).max(10000), audio, cursor, captions, canvas: canvas.optional(), autoZoom: autoZoom.optional() }).strict();
...
export const sourceSchema = z.object({ durationMs: time.positive(), width: finite.int().min(16).max(16384), height: finite.int().min(16).max(16384), fps: finite.min(1).max(240), screen: relativeFile, camera: relativeFile.optional(), microphone: relativeFile.optional(), systemAudio: relativeFile.optional(), cursor: relativeFile.optional(), cameraActiveRanges: z.array(range).optional(), title: z.string().max(1000).optional() }).strict();
...
export const operationSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('clip.trim'), index: finite.int().min(0), sourceStartMs: time, sourceEndMs: time }).strict(),
  ...
  z.object({ type: z.literal('zoom.add'), zoom: zoom.omit({ id: true }) }).strict(), z.object({ type: z.literal('zoom.update'), id, zoom: zoom.omit({ id: true }).partial() }).strict(), z.object({ type: z.literal('zoom.remove'), id }).strict(),
  ...
  z.object({ type: z.literal('autoZoom.update'), settings: autoZoom.partial() }).strict(),
  z.object({ type: z.literal('transcript.text'), id, text: z.string().max(10000) }).strict(),
  z.object({ type: z.literal('transcript.update'), segments: transcript }).strict(), autoZoom.partial().extend({ type: z.literal('zooms.auto') }).strict(),
]);
