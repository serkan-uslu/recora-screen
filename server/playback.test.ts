...
test("scrubbing waits for pause, keeps one seek in flight, chases only the latest target and restores playback", async () => {
  const pause = deferred<void>(),
    requests: { target: number; done: ReturnType<typeof deferred<void>> }[] = [];
  ...
  const playback = createPreviewPlayback(
    async (method, params) => {
      ...
      if (method === "preview.seek") {
        assert(
          Number.isFinite(params!.inputAtMs),
          "seek carries its original input timestamp through the queue",
        );
        const done = deferred<void>();
        requests.push({ target: params!.timeMs as number, done });
  ...
  await playback.poll();
  playback.beginScrub();
  const first = playback.seek(100),
    second = playback.seek(200);
  await tick();
  assert.equal(requests.length, 0, "audio pause completes before any scrub seek");
  pause.resolve();
  await tick();
  assert.deepEqual(
  ...
  const third = playback.seek(300),
    fourth = playback.seek(400),
    finished = playback.endScrub(500);
  assert.equal(
    playback.getSnapshot().timeMs,
    500,
    "playhead responds before native seek completes",
  );
  ...
  assert.equal(playback.getSnapshot().scrubbing, false);
  assert.equal(playback.getSnapshot().playing, true);
...
test("late status never reverses a seek or leaks into a new project; unchanged polling does not notify subscribers", async () => {
  const statuses: ReturnType<typeof deferred<{ timeMs: number; playing: boolean }>>[] = [],
    errors: unknown[] = [];
  const playback = createPreviewPlayback(
    async (method) => {
      if (method === "preview.status") {
        const result = deferred<{ timeMs: number; playing: boolean }>();
  ...
  playback.reset(1000);
  let updates = 0;
  playback.subscribe(() => updates++);
  const stale = playback.poll();
  await playback.seek(700);
  statuses[0]!.resolve({ timeMs: 10, playing: true });
  await stale;
  assert.equal(playback.getSnapshot().timeMs, 700);
  assert.equal(playback.getSnapshot().playing, false);
  const previousProject = playback.poll();
  playback.reset(2000);
  statuses[1]!.resolve({ timeMs: 900, playing: true });
  await previousProject;
  assert.equal(playback.getSnapshot().timeMs, 0);
  assert.equal(playback.getSnapshot().playing, false);
  const count = updates,
    same = playback.poll();
  statuses[2]!.resolve({ timeMs: 0, playing: false });
  await same;
...
test("seek clamps to duration, surfaces failure and continues with the latest target", async () => {
  ...
  playback.reset(1000);
  const a = playback.seek(-50);
  await tick();
  const b = playback.seek(2000);
  first.resolve();
  await Promise.all([a, b]);
  assert.deepEqual(targets, [0, 1000]);
  assert.equal(errors.length, 1);
  playback.setDuration(500);
  await tick();
  assert.equal(playback.getSnapshot().timeMs, 500);
  assert.equal(targets.at(-1), 500);
});
...
test("a delayed play response cannot change playback state after switching projects", async () => {
  const play = deferred<void>(),
    errors: unknown[] = [];
  const playback = createPreviewPlayback(
    async (method) => (method === "preview.play" ? play.promise : {}),
    (error) => errors.push(error),
  );
  playback.reset(1000);
  const toggled = playback.toggle();
  playback.reset(2000);
  play.resolve();
  await toggled;
  assert.equal(playback.getSnapshot().playing, false);
...
test("project switch while scrub playback resumes cannot mark the new project playing", async () => {
  ...
  await playback.poll();
  playback.beginScrub();
  const finish = playback.endScrub(300);
  await tick();
  playback.reset(2000);
  play.resolve();
  await finish;
...
test("delayed project refreshes preserve a project switch, dashboard navigation and newer committed revisions", async () => {
  ...
  for (const destination of [b, null, { ...a, revision: 3 }]) {
    ...
    const refreshing = read.promise.then((next) => {
      current = reconcileProjectRefresh(current, next);
    });
    current = destination;
    read.resolve({ ...a, revision: 2 });
    await refreshing;
