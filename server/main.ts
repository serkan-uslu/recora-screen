...
import { readLines, serveSocket } from "@/server/infrastructure/rpc.js";
    ...
    new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new AppError(
            "NATIVE_TIMEOUT",
            `Native operation ${method} timed out. Check recording or job status before retrying.`,
          ),
        );
      }, 300000);
  ...
  (line) => {
    ...
    if (typeof request.nativeResponse === "string") {
      ...
      if (waiting) {
        ...
        pending.delete(request.nativeResponse);
        if (request.error) {
          waiting.reject(
            new AppError(
              request.error.code ?? "NATIVE_ERROR",
              request.error.message ?? String(request.error),
    ...
    if (typeof request.method !== "string") {
      send({
        id: request.id,
        error: { code: "INVALID_REQUEST", message: "RPC method is required" },
      });
      return;
    }
    ready
      .then(() => service.command(request.method!, request.params))
      .then(
        (result) => send({ id: request.id, result }),
        (error) => send({ id: request.id, error: errorOf(error) }),
      );
  },
  (error) => {
...
ready.catch((error) => {
  console.error(errorOf(error).message);
  process.exit(1);
});
