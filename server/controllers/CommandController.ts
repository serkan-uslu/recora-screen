import { methodSchemas, isReadOnly } from "../contracts/commands.js";
import { AppError, object, id } from "../contracts/validation.js";
...
export class CommandController {
  ...
  async command(method: string, input: unknown = {}): Promise<any> {
    ...
    if (!schema)
      throw new AppError("UNKNOWN_METHOD", `Unknown command: ${method}`);
    ...
    if (this.shuttingDown && method !== "app.canQuit")
      throw new AppError("APP_CLOSING", "The application is closing. Reopen it before making changes.");
    if (method === "app.shutdown") {
      this.shuttingDown = true;
      try { await Promise.all([...this.queues.values()].map(work => work.catch(() => {}))); return await execute(); }
      catch (error) { this.shuttingDown = false; throw error; }
    }
  ...
  private serialize(method: string, params: Record<string, any>, action: () => Promise<unknown>) {
    if (isReadOnly(method) || method === "recording.status" || method === "jobs.cancel" ||
        (method.startsWith("preview.") && !["preview.draft", "preview.load", "preview.reset"].includes(method)))
      return action();
    const keys = [
    ...
    const prior = keys.map(key => this.queues.get(key)?.catch(() => {}));
