...
import { AppError, finite, time } from "../contracts/validation.js";
...
export async function cursorClicks(file: string): Promise<CursorEvent[]> {
  ...
  for await (const chunk of createReadStream(file, { encoding: "utf8" })) {
    for (const char of chunk) {
      if (!depth) {
        ...
        if (char !== "{")
          throw new AppError("INVALID_CURSOR", "Cursor metadata is malformed.");
        depth = 1;
        item = "{";
        continue;
      }
      item += char;
      if (item.length > 4096)
        throw new AppError("INVALID_CURSOR", "Cursor event exceeds 4 KB.");
      if (quoted) {
  ...
  if (depth)
    throw new AppError(
      "INVALID_CURSOR",
      "Cursor metadata ends inside an event.",
    );
  return clicks;
}
