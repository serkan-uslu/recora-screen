...
import { applyEdits } from './domain/edits.js';
import { parseProject, projectSchema } from './contracts/validation.js';
...
async function setup(t: TestContext) {
  ...
  project = await store.mutate(project.id, project.revision, p => {
    p.status = 'ready'; p.source = { durationMs: 10000, width: 1920, height: 1080, fps: 30, screen: 'media/screen.mov' };
    p.edits.segments = [{ startMs: 0, endMs: 10000 }];
  }, false);
  return { store, project, dir: store.dir(project.id) };
}
const times = (project: Project) => project.edits.camera.layouts.map(({ startMs, endMs, x, size }) => ({ startMs, endMs, x, size }));
...
test('camera layouts preserve disjoint source spans, partial overlap, history and source restoration', async t => {
  ...
  let project = await store.mutate(original.id, original.revision, p => applyEdits(p, [
  ...
  project = await store.mutate(project.id, project.revision, p => applyEdits(p, [
    { type: 'camera.layout.set', startMs: 1500, endMs: 2500, settings: { size: 0.4 } },
  ]));
  assert.deepEqual(times(project), [
    { startMs: 1000, endMs: 1500, x: 0.1, size: 0.3 }, { startMs: 1500, endMs: 2000, x: 0.1, size: 0.4 },
    { startMs: 4000, endMs: 4500, x: 0.1, size: 0.4 }, { startMs: 4500, endMs: 5000, x: 0.1, size: 0.3 },
  ]);
  ...
  assert.equal(cameraAt(project, 3000).x, original.edits.camera.x, 'restored source gap retains the base layout');
  applyEdits(project, [{ type: 'camera.layout.remove', startMs: 1250, endMs: 4750 }]);
  assert.deepEqual(times(project), [
    { startMs: 1000, endMs: 1250, x: 0.1, size: 0.3 }, { startMs: 4750, endMs: 5000, x: 0.1, size: 0.3 },
  ]);
...
test('splitting layout IDs remains valid, adjacent targets coalesce, and global edits preserve interval targets', async t => {
  ...
  assert(projectSchema.safeParse(project).success);
  applyEdits(project, [{ type: 'camera.layout.set', startMs: 3000, endMs: 4000, settings: { x: 0.2 } }]);
  assert.equal(project.edits.camera.layouts.length, 1);
  applyEdits(project, [{ type: 'camera.update', settings: { x: 0.9 } }]);
  assert.equal(cameraAt(project, 5000).x, 0.2); assert.equal(cameraAt(project, 8000).x, 0.9);
  assert.throws(() => applyEdits(project, [{ type: 'camera.layout.set', startMs: 0, endMs: 11000, settings: { x: 0.1 } }]), { code: 'INVALID_RANGE' });
  assert.throws(() => applyEdits(project, [{ type: 'camera.layout.set', startMs: 0, endMs: 500, settings: { x: 2 } }]));
});
...
test('camera transitions use output time across cuts and speed, with one centered transition between neighbors', async t => {
  ...
  assert.deepEqual(cameraOutputLayouts(project).map(r => [r.startMs, r.endMs, r.x]), [
    [0, 500, 0], [500, 2000, 1], [2000, 3000, 0.5], [3000, 4000, 0],
  ]);
  ...
  applyEdits(project, [{ type: 'camera.layout.set', startMs: 25, endMs: 50, settings: { x: 0.8 } }]);
  assert(Number.isFinite(cameraAt(project, 25).x));
  assert.equal(cameraAt(project, 37.5).x, 0.8, 'short intervals reach their target before the next transition');
});
...
test('v1 migration archives exact originals and upgrades current, backup and both history stacks once', async t => {
  ...
  const manifest = JSON.stringify({ project: original, undo: [{ ...original, name: 'Undo name' }], redo: [{ ...original, name: 'Redo name' }] });
  const backup = JSON.stringify({ project: { ...original, name: 'Backup name' }, undo: [original], redo: [] });
  ...
  const opened = await Promise.all([fresh.get(project.id), fresh.get(project.id), fresh.read(project.id)]);
  ...
  assert.equal(migrated.undo[0]!.schemaVersion, 2); assert.equal(migrated.redo[0]!.schemaVersion, 2);
  assert.deepEqual(migrated.project.edits.camera.layouts, []);
  assert.equal(JSON.parse(await fs.readFile(path.join(dir, 'project.backup.json'), 'utf8')).project.name, 'Backup name');
...
test('interrupted migration resumes with the already upgraded backup; corruption recovers without losing v2 layouts', async t => {
  const { store, project, dir } = await setup(t);
  await fs.writeFile(path.join(dir, 'project.json'), JSON.stringify({ project: legacy(project), undo: [], redo: [] }));
  await fs.writeFile(path.join(dir, 'project.backup.json'), JSON.stringify({ project, undo: [], redo: [] }));
  const fresh = new ProjectStore(store.root);
  const migrated = await fresh.get(project.id);
  let updated = await fresh.mutate(project.id, migrated.revision, p => applyEdits(p, [{ type: 'camera.layout.set', startMs: 1000, endMs: 5000, settings: { x: 0.1 } }]));
  updated = await fresh.mutate(project.id, updated.revision, p => { p.name = 'Latest'; });
  await fs.writeFile(path.join(dir, 'project.json'), '{broken');
  const recovered = await fresh.get(project.id);
  assert.equal(recovered.recovered, true); assert.equal(recovered.edits.camera.layouts[0]!.x, 0.1);
...
test('unsupported project or history versions never fall back or overwrite original documents', async t => {
  ...
  for (const document of [
    ...
    await fs.writeFile(path.join(dir, 'project.backup.json'), JSON.stringify({ project, undo: [], redo: [] }));
    await assert.rejects(new ProjectStore(store.root).get(project.id), { code: 'UNSUPPORTED_PROJECT_VERSION' });
    await assert.rejects(store.persist({ project, undo: [], redo: [] }), { code: 'UNSUPPORTED_PROJECT_VERSION' });
    assert.equal(await fs.readFile(path.join(dir, 'project.json'), 'utf8'), text);
  }
  assert.throws(() => parseProject({ ...legacy(project), edits: { ...project.edits } }), /layouts/, 'v1 schema does not accept unrecognized v2 fields');
});
