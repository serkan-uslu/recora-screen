...
export function createMcpServer(
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: Object.entries(toolMethods).map(([name, method]) => {
      const schema = z.toJSONSchema(methodSchemas[method]!, { unrepresentable: "any" });
      ...
      return {
        name,
        description: `${method}: ${descriptions[method] ?? "Run the corresponding desktop application command. Returns current state or a background job."}`,
        inputSchema: { ...schema, type: "object" as const },
        annotations: {
          readOnlyHint: isReadOnly(method),
          destructiveHint: method === "project.delete",
          idempotentHint: isReadOnly(method),
...
const descriptions: Record<string, string> = {
  ...
  "project.open":
    "Read a project, revision and source-time edit state. Source media remains immutable.",
  "timeline.apply":
    "Apply sequential edits atomically. Times are OUTPUT milliseconds except clip.trim sourceStartMs/sourceEndMs and source.restore startMs/endMs explicitly use SOURCE time. Speed is 0.25–8; transcript.text corrects words by id without changing timing. canvas.update and autoZoom.update merge settings; zooms.auto regenerates from click/drag/typing activity. Normalized x/y/size are 0–1. Cut multiple ranges in reverse order to avoid shifting later ranges. Stale expectedRevision is rejected. One batch is one undo step.",
  "ai.cleanSilence":
    "Analyze source microphone RMS audio and protect audible system audio. Default returns suggested OUTPUT cut ranges and operations; apply=true commits an undoable edit. Returns Job; poll jobs_get.",
  "ai.transcribe":
    "Run offline Whisper transcription with an already downloaded model. Source timestamps are preserved. Automatically enables editable captions. Returns Job.",
  "ai.assistant":
    "Send project metadata and transcript to the selected BYOK OpenAI/Anthropic provider and let it edit via validated project tools. This is the cloud-assisted feature; no video upload. Returns Job.",
  "ai.models/download":
    "Download one pinned SHA-256 verified multilingual Whisper model. base is 148 MB, small is 488 MB. Returns Job.",
  "recording.camera":
    "visible changes the live bubble, enabled toggles camera hardware capture (off gaps cannot be restored), shape changes circle/square. For reversible post-recording visibility use timeline_apply camera.hide.",
  "project.delete":
    "Move an idle project to the operating-system Trash. Active recordings and jobs prevent deletion.",
  "export.start":
    "Render a project snapshot to a new external MP4 path. Omit width and height to follow the canvas aspect ratio; quality selects 720, 1080 (default), or 4k. Custom dimensions must be supplied together, even, and at most 3840 on either axis. Never overwrite an existing file or source media. Returns Job.",
  "transcript.export":
    "Return SRT/VTT text aligned to the edited timeline; optionally write to a new external file.",
};
