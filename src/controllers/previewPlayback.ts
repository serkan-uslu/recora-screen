...
export function createPreviewPlayback(send: Send, onError: (error: unknown) => void) {
  ...
  const update = (patch: Partial<PlaybackState>) => {
    if (Object.entries(patch).every(([key, value]) => state[key as keyof PlaybackState] === value))
      return;
    state = { ...state, ...patch };
  ...
  const drain = (): Promise<void> => {
    ...
    const work = (async () => {
      ...
      while (pending) {
        ...
        try {
          await send("preview.seek", { timeMs: target.timeMs, inputAtMs: target.inputAtMs });
        } catch (error) {
          if (target.scope === scope) onError(error);
        }
      }
    })();
    running = work;
  ...
  return {
    ...
    reset(duration: number) {
      ...
      pending = null;
      resume = false;
      update({ timeMs: 0, playing: false, scrubbing: false, geometry: undefined });
    },
    setDuration(duration: number) {
      total = duration;
      if (state.timeMs > total) void seek(total);
    },
    seek,
    async poll() {
      const token = epoch,
        currentScope = scope;
      try {
        const result = (await send("preview.status")) as Partial<PlaybackState> | null;
        if (token !== epoch || currentScope !== scope || state.scrubbing || running || pending)
          return;
        if (result)
          update({
            timeMs: result.timeMs ?? state.timeMs,
            playing: Boolean(result.playing),
            geometry: result.geometry,
          });
      } catch {
        /* Initial loading reports its own error. */
      }
    },
    beginScrub() {
    ...
    async endScrub(value: number, _cancelled = false) {
      ...
      if (shouldResume) {
        try {
          await send("preview.play");
          if (token === scope) update({ playing: true });
        } catch (error) {
          if (token === scope) onError(error);
        }
      }
    },
    async toggle() {
      ...
      try {
        await send(next ? "preview.play" : "preview.pause");
        if (token === epoch && currentScope === scope) update({ playing: next });
      } catch (error) {
        if (currentScope === scope) onError(error);
      }
    },
  };
}
