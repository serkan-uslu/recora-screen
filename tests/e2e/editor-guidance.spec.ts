...
test("editor explains first steps and reports actual project saves including failures", async ({
  ...
  try {
    ...
    delaySave = true;
    await page.getByLabel("Project options", { exact: true }).focus();
    await page.getByLabel("Project options", { exact: true }).press("Space");
    ...
    delaySave = false;
    failSave = true;
    await page.getByLabel("Project options", { exact: true }).click();
    await page.getByRole("button", { name: "Save now" }).click();
    ...
    await expect(page.getByRole("button", { name: "Quick start", exact: true })).toBeInViewport();
